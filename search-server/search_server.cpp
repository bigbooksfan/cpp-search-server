...
const std::map<std::string, double>& SearchServer::GetWordFrequencies(int document_id) const
{
    ...
    //    if (word.second.count(document_id) != 0) {
    //        ret.emplace(word.first, word.second.at(document_id));
    //    }
    //}
...
void SearchServer::RemoveDocument(int document_id)
{
    ...
    //ids_.erase(std::find(ids_.begin(), ids_.end(), document_id));                   // delete first
    //ids_.erase(std::remove(ids_.begin(), ids_.end(), document_id),ids_.end());      // delete each

    //if (words_of_docs_.count(document_id)) {
    //    words_of_docs_.erase(document_id);
    //}
}
...
//    if (pre_end != words_of_docs_.begin()) {
//        --pre_end;
//    }
//    else {
//        throw std::out_of_range("Something wrong with (words_of_docs_) size!");
//    }
//
...
//            ++It_fast;
//            
//            if ((*It_slow).second.size() != (*It_fast).second.size()) {
//                continue;
...
SearchServer::SearchServer(const std::string& stop_words) {          // explicit
    if (!IsValidWord(stop_words)) {
        throw std::invalid_argument("Special symbol in constructor (Yandex method)");
    }
...
SearchServer::Query SearchServer::ParseQuery(const std::string& text) const {
    ...
    for (const std::string& word : SplitIntoWords(text)) {
        ...
        if (query_word.is_minus && query_word.data[0] == '-') {
